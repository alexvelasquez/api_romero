...
class PlayerController extends FOSRestController
{
    ...
    public function editPlayer(Request $request,$id){
        ...
        try {
            ...
            $category = !empty($request->request->get("category")) ? $em->getRepository("App:Category")->find($request->request->get("category")) : null;
            if (!is_null($name) && !is_null($lastName) && !is_null($dni) && !is_null($birthDate) && !is_null($category)) {
    ...
    public function addPlayer(Request $request) {
        ...
        try {
           ...
           $birthDate = !empty($request->request->get("birthDate")) ? new \DateTime($request->request->get("birthDate")) : null;
           $category = !empty($request->request->get("category")) ? $em->getRepository("App:Category")->find($request->request->get("category")) : null;
           $phone = $request->request->get("phone",null);
           if (!is_null($name) && !is_null($lastName) && !is_null($category)) {
               $player = new Player(ucwords($name),ucwords($lastName),$birthDate,$dni,$phone,$category);
               $em->persist($player);
               $em->flush();
     ...
     * @SWG\Tag(name="Player")
     */
    public function playersForCategories($categoryId) {
        $serializer = $this->get('jms_serializer');
        $em = $this->getDoctrine()->getManager();
        $players = [];
        try {
